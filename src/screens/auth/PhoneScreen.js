...
export default function PhoneScreen({ navigation, route }) {
  ...
  const handleTestClient = async () => {
    ...
    try {
      ...
      if (error) throw error;
    } catch (e) { setError('Test error: ' + e.message); }
  ...
  return (
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={s.inner}>
        ...
        <TextInput style={s.input} placeholder="example@mail.ru" placeholderTextColor="#9B7B6A"
          keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail} />
        {error ? <Text style={s.error}>{error}</Text> : null}
        <TouchableOpacity style={s.btn} onPress={handleSend} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>{t.sendCode}</Text>}
        </TouchableOpacity>
        <View style={s.testSection}>
          ...
          <TouchableOpacity style={s.testBtn} onPress={handleTestClient} disabled={testLoading}>
            {testLoading ? <ActivityIndicator color="#C9A84C" size="small" />
...
const s = StyleSheet.create({
  ...
  testBtn:{backgroundColor:'rgba(201,168,76,0.1)',borderWidth:1,borderColor:'rgba(201,168,76,0.3)',borderRadius:12,padding:13,alignItems:'center'},
