...
export default function DashboardScreen({ navigation }) {
  ...
  const handleSave = async () => {
    if (!selectedDate || selectedTimes.length === 0) {
      Alert.alert('Ошибка', 'Выберите дату и хотя бы одно время'); return;
    }
    setSaving(true);
    try {
      for (const time of selectedTimes) {
        await saveSlots(selectedDate, time, addMinutes(time, 90));
      }
      Alert.alert('✅ Сохранено!', `Слоты на ${selectedDate} созданы.`);
      setSelectedTimes([]); setSelectedDate(null);
  ...
  const handleSwitchToClient = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithPassword({ email: 'test@animi.app', password: 'test123456' });
    ...
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      <View style={s.header}>
        ...
        <View style={s.headerBtns}>
          <TouchableOpacity style={s.testSwitchBtn} onPress={handleSwitchToClient} disabled={switchingTest}>
            {switchingTest ? <ActivityIndicator color="#C9A84C" size="small" />
              : <Text style={s.testSwitchText}>👤</Text>}
          </TouchableOpacity>
      ...
      <View style={s.tabs}>
        <TouchableOpacity style={[s.tab, tab==='today' && s.tabActive]} onPress={() => setTab('today')}>
          <Text style={[s.tabText, tab==='today' && s.tabTextActive]}>📋 Сессии сегодня</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.tab, tab==='schedule' && s.tabActive]} onPress={() => setTab('schedule')}>
          <Text style={[s.tabText, tab==='schedule' && s.tabTextActive]}>📅 Мой график</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={s.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
            {days.map((d, i) => {
              ...
              return (
                <TouchableOpacity key={i} style={[s.dayBtn, isSelected && s.dayBtnActive]} onPress={() => setSelectedDate(ds)}>
          ...
          {selectedDate && (
            <>
              <Text style={s.scheduleLabel}>Выберите время (сессия 1.5 ч):</Text>
              <View style={s.timesGrid}>
                {TIMES.map(time => (
                  <TouchableOpacity key={time}
                    style={[s.timeBtn, selectedTimes.includes(time) && s.timeBtnActive]}
                    onPress={() => toggleTime(time)}>
                    <Text style={[s.timeTxt, selectedTimes.includes(time) && s.timeTxtActive]}>{time}</Text>
                    <Text style={[s.timeEnd, selectedTimes.includes(time) && s.timeTxtActive]}>до {addMinutes(time,90)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <TouchableOpacity style={s.saveBtn} onPress={handleSave} disabled={saving}>
                {saving ? <ActivityIndicator color="#0F2447" /> : <Text style={s.saveBtnText}>Сохранить</Text>}
              </TouchableOpacity>
            </>
          )}
        </View>
...
const s = StyleSheet.create({
  ...
  testSwitchBtn:{backgroundColor:'rgba(201,168,76,0.12)',borderWidth:1,borderColor:'rgba(201,168,76,0.25)',borderRadius:10,paddingHorizontal:10,paddingVertical:6},
  testSwitchText:{fontSize:16},
  ...
  scheduleLabel:{color:'rgba(255,255,255,0.6)',fontSize:12,fontWeight:'600',marginBottom:12,letterSpacing:0.5},
  ...
  timeBtn:{backgroundColor:'rgba(255,255,255,0.07)',borderRadius:12,padding:12,alignItems:'center',minWidth:'47%'},
  ...
  saveBtn:{backgroundColor:'#C9A84C',borderRadius:14,padding:16,alignItems:'center'},
